...
int main()
{
	...
	//namedWindow("edge", WINDOW_GUI_NORMAL);
	...
	while (1)
	{
		...
		else
		{
			...
			//line(original, Point(original.cols / 2 + offset, original.rows - 5), Point(original.cols / 2, original.rows - 5), 250, 5);
			line(colorMask, Point(colorMask.cols / 2 + offset, colorMask.rows - 5), Point(colorMask.cols / 2, colorMask.rows - 5), 0, 5);
